...
class _Scene {
  ...
  /// Opened at a table (its Animated shelf and its seat's chips), rather than
  /// over the lobby.
  final bool atTable;
  ...
  /// Scrolled to the foot of the shelf: the Premium Packages.
  final bool toEnd;
}

const _scenes = [
  _Scene('01-chips', StoreTab.chips),
  _Scene('02-premium', StoreTab.chips, toEnd: true),
  _Scene('03-diamonds', StoreTab.diamonds),
  _Scene('04-hammers', StoreTab.hammers),
  _Scene('05-missiles', StoreTab.missiles),
  _Scene('06-pictures', StoreTab.pictures),
  _Scene('07-tables', StoreTab.tables),
...
/// The files the fake catalogues point at, primed into the picture cache: the
/// browser client's animal SVGs, the generated table SVGs and the app's own
/// Lottie animations. Every URL is absolute, so GameState.absoluteUrl leaves
/// it alone.
const _repo = '../';
...
Future<void> _primePictures() async {
  ...
  for (final (url, file) in const [
    ...
    ('envelope.json', 'Quick message.json'),
...
/// Every state a picture tile can be in: free, worn (id 3), owned for good,
/// a rental with time left, and locked in each wallet with and without a
/// term — plus a long name and a large chip price.
List<ProfilePicture> _catalogue() => [
  ...
  _picture(
    ...
    days: 10,
    owned: true,
    expiresAt: _inDays(6, hours: 4),
  ),
  _picture(4, 'Tiger', 'tiger.svg', cost: 500000, owned: true),
  _picture(5, 'Fox', 'fox.svg', cost: 25000),
  _picture(6, 'Wolf', 'wolf.svg', cost: 1000000, hours: 1),
  _picture(7, 'Bodybuilder', 'monkey.svg', cost: 500000000, days: 50),
  _picture(
    8,
    'Orange Ballerina',
    'horse.svg',
  ...
  // The animated shelf a seated player shops from.
  _picture(
    13,
    'Hammer Time',
    'hammer.json',
...
/// The table shelf: one laid (id 2), one owned and waiting, and locked ones
/// in each wallet.
List<TablePicture> _tableCatalogue() => [
  _table(1, 'Royal Sapphire', 'royal-sapphire', cost: 100000),
  _table(
    2,
    'Emerald Lattice',
    'emerald-lattice',
  ...
  _table(6, 'Carbon Weave', 'carbon-weave', cost: 150000),
];
...
/// The lobby's state: the menu, Priya's account, and both catalogues.
GameState _lobbyState(AppLang lang) {
...
/// and catalogues.
GameState _tableState(AppLang lang) {
...
/// the test engine draws each as an empty box.
...
/// The theme with the fallbacks a phone has: the Indic scripts' Noto fonts
/// for a Hindi pass, and colour emoji always.
ThemeData _theme(bool dark, AppLang lang) {
  final base = dark
      ? AppTheme.dark(sound: false)
      : AppTheme.light(sound: false);
  final scripts = lang == AppLang.english
      ? const <String>[]
      : scriptFonts.keys.toList();
  return base.copyWith(
    textTheme: base.textTheme.apply(
      fontFamilyFallback: [...scripts, _emojiFamily],
    ),
  );
}
...
Future<void> _shoot(
  ...
  final state = scene.atTable ? _tableState(shot.lang) : _lobbyState(shot.lang);
  ...
  await settle(900);
  if (scene.toEnd) {
    // The store's body: the one vertical scroll view with somewhere to go.
