...
// tablePictureLapsed tells the table a seated player's lapsed cloth has come
// off (Deps.TablePictureLaid with nil → RoomManager.SetPlayerTablePicture →
// Table.SetTablePicture), so the seat's copy goes the moment the sweep finds
// the rental over and every viewer's felt re-ranks at once. Without it the
// account would say tablePicture:null while the table went on showing a
// picture the chips no longer pay for, until that player left or laid
// another. For a player in the lobby the hook does nothing, and their next
// seat reads the account, which no longer carries it (userFromAt).
func (h *Handler) tablePictureLapsed(userID string) {
	if h.deps.TablePictureLaid != nil {
		h.deps.TablePictureLaid(userID, nil)
	}
}
...
// db.MissilePacks (1 missile for 10 diamonds, 5 for 48, 10 for 90, 20 for 170).
//
