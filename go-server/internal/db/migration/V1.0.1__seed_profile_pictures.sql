...
-- RENTAL, and the coin-priced terms rise with the price — 3 days at 25k, 4 at
-- 50k, 5 at 1L, 7 at 2L — so the dear ones are better value per day as well as
-- rarer. The free pair never lapses.
--
-- Every row states asset_format (how a client renders asset_url: IMAGE, SVG,
-- LOTTIE or RIVE) and currency (which wallet pays: COIN is chips, DIAMOND is
-- users.diamond). The animals are hosted bitmaps priced in coins. Orange
-- Ballerina is a Lottie animation priced at one DIAMOND for a 100-day rental —
-- one diamond being exactly what every new account starts with.
--
...
-- because lh3 only serves images. Change any of it with
