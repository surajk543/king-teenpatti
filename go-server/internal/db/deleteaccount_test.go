...
func TestDeletingAnAccountErasesWhatIdentifiesThePlayer(t *testing.T) {
	...
	// The worn picture is a catalogue id now, not the free-text avatar_choice
	// this test was written against — that column was migrated away on
	// 12 Sep 2026 (CLAUDE.md §7.3).
	var activePictureID *int64
	var providerUserID string
	var deletedAt int64
	if err := f.d.Pool.QueryRow(f.ctx,
		`SELECT display_name, email, avatar_url, active_picture_id, provider_user_id, deleted_at
		   FROM users WHERE id = $1`, u.ID).
		Scan(&name, &email, &avatarURL, &activePictureID, &providerUserID, &deletedAt); err != nil {
		t.Fatal(err)
	}
	if name != db.DeletedDisplayName {
		t.Fatalf("display_name = %q, want %q", name, db.DeletedDisplayName)
	}
	for label, value := range map[string]*string{"email": email, "avatar_url": avatarURL} {
		if value != nil {
			t.Errorf("%s survived deletion: %q", label, *value)
		}
	}
	if activePictureID != nil {
		t.Errorf("active_picture_id survived deletion: %d", *activePictureID)
	}
	if deletedAt == 0 {
