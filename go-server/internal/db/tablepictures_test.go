...
func TestAChipPricedTablePictureIsBoughtLaidAndRenewed(t *testing.T) {
	...
	// Before any sweep has run, the account already reads with no picture
	// laid (userFromAt tests the expiry in its join, 23 Sep 2026): the
	// account is what every seat is built from, and the whole table shows a
	// laid picture, so a lapsed one must never reach a new seat.
	if lapsed := f.find(user.ID); lapsed.TablePicture != nil || lapsed.Player().TablePicture != nil {
		t.Fatalf("a lapsed rental still reads as laid before the sweep: %+v", lapsed.TablePicture)
	}
	if f.laid(user.ID) != sapphire.ID {
