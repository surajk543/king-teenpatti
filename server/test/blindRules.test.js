...
test('a player may see their cards when it is not their turn', () => {
  ...
  assert.doesNotThrow(() => table.act(waiting, ACTION.SEE));
  assert.equal(table.findSeat(waiting).isBlind, false);
});

test('seeing out of turn does not hand the player the turn', () => {
  const { table, seat, advance } = makeTable();
  seat('alice');
  seat('bob');
  advance(baseConfig.nextHandDelayMs);
  ...
  table.act(waiting, ACTION.SEE);
  assert.equal(turnUser(table), onTurn, 'the turn stays where it was');
});

test('a player still cannot bet out of turn after looking', () => {
  const { table, seat, advance } = makeTable();
  seat('alice');
  seat('bob');
  advance(baseConfig.nextHandDelayMs);
...
test('the cards turn face up after the capped number of blind bets', () => {
  const { table, seat, advance } = makeTable();
  seat('alice');
  seat('bob');
  advance(baseConfig.nextHandDelayMs);
  ...
  for (let move = 1; move <= baseConfig.maxBlindMoves; move++) {
    ...
    while (turnUser(table) !== player) table.act(turnUser(table), ACTION.CHAAL);
...
test('the capped bet is itself still charged at the blind rate', () => {
  const { table, seat, advance } = makeTable();
  seat('alice');
  seat('bob');
  advance(baseConfig.nextHandDelayMs);
...
test('a player who looked early is never auto-seen', () => {
  const { table, seat, advance } = makeTable();
  seat('alice');
  seat('bob');
  advance(baseConfig.nextHandDelayMs);
...
test('the counter starts again on the next hand', () => {
  const { table, seat, advance } = makeTable();
  seat('alice');
  seat('bob');
  advance(baseConfig.nextHandDelayMs);
...
test('a seat reports its last bet as well as its running total', () => {
  const { table, seat, advance } = makeTable();
  seat('alice');
  seat('bob');
  advance(baseConfig.nextHandDelayMs);
...
test('the last bet is cleared when the next hand is dealt', () => {
  const { table, seat, advance } = makeTable();
  seat('alice');
  seat('bob');
  advance(baseConfig.nextHandDelayMs);
