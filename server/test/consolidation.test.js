...
test('two tables with one player each are merged into one', () => {
  ...
  const moves = rooms.consolidateTables();
...
test('the merged players can now actually start a hand', () => {
  const { rooms, advance } = makeRooms();

  singleTable(rooms);
  singleTable(rooms);
  rooms.consolidateTables();
...
test('the player is moved onto the longest-standing table', () => {
  const { rooms } = makeRooms();
...
test('three lone players end up at the same table', () => {
  const { rooms } = makeRooms();
...
test('a move is announced so the client can follow it', () => {
  const { rooms } = makeRooms();
...
test('the room index follows the player to their new table', () => {
  const { rooms } = makeRooms();
...
test('a table with a hand in progress is never disturbed', () => {
  const { rooms, advance } = makeRooms();
...
test('a lone player is not moved while their own table has a live hand', () => {
  ...
  rooms.leave(table.occupiedSeats[0].userId, 'left');
...
test('tables of different stakes are never merged', () => {
  const { rooms } = makeRooms();

  singleTable(rooms, { bootAmount: 200 });
  singleTable(rooms, { bootAmount: 5000 });

  assert.equal(rooms.consolidateTables().length, 0, 'a player keeps the stake they chose');
  assert.equal(rooms.tables.size, 2);
});

test('blind and seen tables are never merged', () => {
  const { rooms } = makeRooms();

  singleTable(rooms, { category: TABLE_CATEGORY.BLIND });
  singleTable(rooms, { category: TABLE_CATEGORY.SEEN });

  assert.equal(rooms.consolidateTables().length, 0, 'a player keeps the category they chose');
  assert.equal(rooms.tables.size, 2);
});

test('private tables are left alone', () => {
  const { rooms } = makeRooms();
  ...
  assert.equal(rooms.consolidateTables().length, 0, 'a private room is joined on purpose');
  assert.equal(rooms.tables.size, 2);
});

test('a full destination stops taking players', () => {
  const { rooms } = makeRooms();
...
test('a single lone table has nothing to merge with', () => {
  const { rooms } = makeRooms();
  singleTable(rooms);

  assert.equal(rooms.consolidateTables().length, 0);
  assert.equal(rooms.tables.size, 1);
});

// ------------------------------------------------ the start countdown

test('the table announces when the next hand starts', () => {
  const { rooms } = makeRooms();
...
test('leaving a table triggers a merge without waiting for the sweep', () => {
  const { rooms } = makeRooms();
  ...
  rooms.leave(a.occupiedSeats[0].userId, 'left');
