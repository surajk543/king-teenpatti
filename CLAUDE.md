...
Orange Ballerina (a LOTTIE at 1 DIAMOND for 100 days); `V1.0.2__seed_animated_pictures.sql` adds Butterfly Flapping (a LOTTIE, same price, sort_order 170) and Toucan Flying (a landscape 1920×1080 LOTTIE, same price, sort_order 180) — each script inserts with `ON CONFLICT (asset_url) DO NOTHING`, so re-pricing or retiring one is an UPDATE
