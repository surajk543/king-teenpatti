...
└── flutter-client/
    ...
    │   ├── theme/table_theme.dart  the table's type scale and tokens: TableType/SeatType, TableSpace, TableScrim, TableAmbient (§8.4); widgets/edge_fade.dart EdgeFade
...
- **The table polish** (owner's brief, 24 Sep 2026: "a polish pass, not a redesign" — type scale, spacing, clipping,
  ...
  **Scrims and ambient light** — `TableScrim.drawer` (ink900 at 0.40, `drawerScrimColor` on both table Scaffolds),
  ...
  and its Leave key in the error colour, every other question's key gold. **Chat** — a line the table wrote (no
