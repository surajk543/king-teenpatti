...
| Personal info → **User IDs** | Yes | No | No | App functionality, Account management | **Google sign-in, and missed for the same reason the email row was.** `providers.go` reads the `sub` claim out of the Google ID token and `users.go` persists it as `users.provider_user_id` — a Google account identifier, sent from the device, stored against a named person. Optional because a guest's `provider_user_id` is a hash of the device id instead, declared under Device or other IDs. Added 22 Sep 2026, while filling step 3 of the form. |
