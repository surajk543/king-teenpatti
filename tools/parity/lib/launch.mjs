/**
 * Starts the Go game server binary on a free port with a throwaway Postgres
 * schema, waits for /health, and tears it down (SIGTERM, then DROP SCHEMA).
 * Shared by parity.mjs and parity-diff.mjs; nothing in here is test-specific.
 *
 * The Node server this harness was first written against was removed from the
 * repository once the Go port had matched it (141/141 suites, identical event
 * streams); `git log -- server/` has it if a Node-vs-Go diff is ever needed again.
 */
...
export const repoDir = path.resolve(toolsDir, '..');
export const goServerDir = path.join(repoDir, 'go-server');
...
export const resolveGoBinary = (bin) => {
  const goBinary = path.resolve(bin ?? path.join(goServerDir, 'bin', 'gameplay'));
  if (!fs.existsSync(goBinary)) {
    throw new Error(`Go binary not found at ${goBinary}. Build it first:\n`
      + `  cd ${goServerDir} && go build -o bin/gameplay ./cmd/gameplay   (or bash ops/build.sh)\n`
      + 'or pass --bin <path>.');
  }
  return goBinary;
};
...
export const startServer = async ({
  ...
  if (target !== 'go') throw new Error(`target must be go (got ${target}); the Node server is no longer in the repository`);
  const goBinary = resolveGoBinary(bin);
