 ...
 *
 *   node parity-diff.mjs --a go --b http://127.0.0.1:3000 --schema-b public   # spawn one, compare with a live server
...
/** `spec` is a URL or the target name `go` (spawns a fresh binary). */
const resolveSide = async (label, spec, schemaArg) => {
  ...
  if (spec !== 'go') throw new Error(`--${label} must be a URL or "go" (got ${spec}); the Node server was removed once the port matched it`);
